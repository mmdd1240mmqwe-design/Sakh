...
================================================================================
راه‌اندازی سریع:
    pip install rubka python-dotenv
...
# =============================================================================
# ایمپورت‌های خارجی (کتابخونه‌های لازم -> پایین‌تر تو requirements.txt هم هست)
# =============================================================================
import os
import re
import time
...
# ==============================================================================
# بخش ۳: محتوای ثابت (مود، فروشگاه، معما و...)
# ==============================================================================
# =============================================================================
...
# ==============================================================================
# بخش ۵: رپر توابع کتابخونه rubka (ریپلای، حذف عضو/پیام و...)
# ==============================================================================
bot_message_ids = set()
...
async def try_handle_guess_input(reply, uid, u, text_raw):
    """اگه کاربر تو یه بازی حدس‌عدد فعال باشه و پیامش یه عدده، اینجا پردازش میشه.
    برمی‌گردونه: True اگه پردازش کرد، False اگه ربطی نداشت (بذار بقیه‌ی
    روتر ادامه بده)."""
    if uid not in active_guess:
        return False
    if not text_raw.strip().lstrip("-").isdigit():
        return False
    ...
    if guess == state["answer"]:
        ...
        add_coins(uid, reward)
        add_xp(uid, 15)
        with_str = f"تو {state['tries']} بار حدس زدی"
        del active_guess[uid]
        await reply(f"🎉 آفرین! درست حدس زدی ({with_str})\n💰 جایزه: {reward} سکه | ✨ +15 XP")
...
async def dispatcher(bot: Robot, message: Message):
    ...
    # =========================================================================
    # ---- بازی حدس عدد (اگه فعال باشه، اولویت با ادامه‌ی بازیه) ----
    # =========================================================================
    if await try_handle_guess_input(reply, uid, u, text_raw):
        return
    ...
    if "فروشگاه" in text_raw and "پنالتی" not in text_raw and "رولت" not in text_raw:
